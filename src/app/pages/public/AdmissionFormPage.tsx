...
import { registerAdmissionStudent } from "../../features/students/admissionApi";
...
const DECLARATIONS = [
  "I am applying for admission to Karthikeyan Analysis Study Circle.",
  "All the information I have provided in this form is true and accurate.",
  "I understand that my account will be reviewed and activated by the admin.",
  "I will not share my login credentials with anyone.",
  "I will follow all rules and guidelines of the institution.",
];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (photoFile) {
        ...
        const fileRef = ref(storage, `admissionPhotos/${safeName}`);
      ...
      const result = await registerAdmissionStudent({
        name: form.name.trim(),
        ...
        batchApplied: form.batchApplied.trim() || undefined,
      ...
      // Navigate to success page, passing all data needed for PDF
      navigate("/public/admission-success", {
        replace: true,
        state: {
          result,
          student: {
            ...
            educationalQualification: qual,
            subjects: form.subjects,
            batchApplied: form.batchApplied.trim(),
            isKarthikeyanStudent: form.isKarthikeyanStudent === "Yes",
            karthikeyanYear: form.karthikeyanYear,
            photoUrl,
          },
        },
      });
      ...
      <div className="max-w-2xl mx-auto">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-slate-500 mt-1 text-sm">
            Fill in your details below. Your login credentials will be
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg p-6 md:p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <Input
                ...
                onChange={(e) => setForm({ ...form, nativeDistrict: e.target.value })}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <Label>Batch Applied For (optional)</Label>
              <Input
                value={form.batchApplied}
                onChange={(e) => setForm({ ...form, batchApplied: e.target.value })}
          ...
          <div>
            <Label>
              Educational Qualification *{" "}
              <span className="text-xs font-normal text-slate-500">(select one or both)</span>
            </Label>
              ...
              {(["Yes", "No"] as const).map((opt) => (
                ...
                >
                  <input
                    ...
                    onChange={() => setForm({ ...form, isKarthikeyanStudent: opt, karthikeyanYear: "" })}
                    className="sr-only"
                  />
                  {opt}
                </label>
          ...
          >
            {submitting ? "Submitting Application…" : "Submit Application & Get Credentials"}
          </Button>

          <p className="text-center text-xs text-slate-400">
            Your account will be reviewed by the admin before activation. You will receive your
            login credentials on this page immediately after submission.
          </p>
        </form>
      </div>
    </div>
