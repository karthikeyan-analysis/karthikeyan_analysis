...
export default function LiveClassManagement() {
  ...
  const flattenedRecordingsList = useMemo(() => {
    ...
    for (const cls of readyRecordingsList) {
      ...
      if (cls.recordings && cls.recordings.length > 0) {
        cls.recordings.forEach((rec, idx) => {
          list.push({
            cls,
            rec,
            partLabel: cls.recordings!.length > 1 ? ` (Part ${idx + 1})` : "",
            key: rec.key,
          });
        });
      } else if (cls.recordingKey) {
        list.push({
          cls,
          partLabel: "",
  ...
  const handleWatchPreview = async (
    ...
    // Step 1: Try R2 presigned URL only if we have a recording key (always fresh, 2h TTL, CORS-allowed)
    if (targetKey) {
      try {
        ...
        console.warn(
          "R2 presigned playback URL generation notice:",
          e?.message ?? e,
        );
      }
    }

    // Step 2: Fallback to stored download URL when no R2 key or R2 call failed
    if (!targetUrl) {
  ...
  const handleDownloadRecording = async (
    ...
    // Use actual recording key — never fall back to cls.id (which is NOT a recording key)
    const actualRecKey = rec?.key || cls.recordingKey;
    const targetKey = actualRecKey || cls.id; // only for setDownloadingId tracking
    ...
    try {
      ...
      // Step 1: Request R2 presigned download URL only when we have a real recording key
      if (actualRecKey) {
        try {
          const { url } = await requestRecordingPlaybackUrl(cls.id, {
            disposition: "attachment",
            filename,
            recordingKey: actualRecKey,
          });
      ...
      // Step 2: Fall back to stored download URLs
      if (!downloadUrl && rec?.downloadUrl) {
        downloadUrl = rec.downloadUrl;
      }
      if (!downloadUrl && !rec && cls.recordingDownloadUrl) {
        downloadUrl = cls.recordingDownloadUrl;
      }

      // Step 3: Last-resort Firebase Storage direct URL
      if (!downloadUrl && actualRecKey) {
        try {
          const sRef = storageRef(storage, actualRecKey);
      ...
      if (downloadUrl) {
        await triggerInstantDownload(downloadUrl, filename);
      } else {
  ...
  const handleDeleteRecording = async (
    ...
    try {
      if (rec && cls.recordings && cls.recordings.length > 1) {
        const remaining = cls.recordings.filter((r) => r.key !== rec.key);
        const lastRec = remaining[remaining.length - 1];
        await updateLiveClass(cls.id, {
          recordings: remaining,
          recordingKey: lastRec?.key || (deleteField() as any),
          recordingDownloadUrl: lastRec?.downloadUrl || (deleteField() as any),
          recordingDurationSec: lastRec?.durationSec || (deleteField() as any),
          recordingSizeBytes: lastRec?.sizeBytes || (deleteField() as any),
        });
      } else {
        await updateLiveClass(cls.id, {
