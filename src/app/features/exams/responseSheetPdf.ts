...
export function buildResponseSheetHtml({
  ...
  function buildPartHeader(
    ...
    let marks = 0;
    for (const i of idxs) {
      const st = questionStatuses[i]!;
      if (st.status !== "Unanswered") attempted++;
    ...
    return `<div class="part-header">
      <div class="part-header-title">${escapeHtml(label)}</div>
      <div class="part-header-stats">
        <span>Attempted: <strong>${attempted}</strong></span>
        <span>Correct: <strong>${correct}</strong></span>
        <span>Wrong: <strong>${wrong}</strong></span>
...
<head>
  ...
  <style>
    *{box-sizing:border-box}
    ...
    .part-header-stats{display:flex;gap:14px;flex-wrap:wrap;font-size:10px;color:#4338ca}
    .part-header-stats strong{color:#312e81}
