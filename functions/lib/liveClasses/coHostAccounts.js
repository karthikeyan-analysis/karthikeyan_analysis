...
exports.resetCoHostPassword = exports.deleteCoHost = exports.createCoHost = void 0;
const admin = __importStar(require("firebase-admin"));
const https_1 = require("firebase-functions/v2/https");
async function assertFullAdmin(uid) {
    ...
    if (!isAdmin) {
        throw new https_1.HttpsError("permission-denied", "Only admins can manage co-hosts.");
    }
    // Co-host accounts cannot create/delete other co-hosts.
    if (adm?.kind === "cohost" || user?.adminKind === "cohost") {
        throw new https_1.HttpsError("permission-denied", "Co-hosts cannot manage other co-host accounts.");
    }
}
 ...
 */
exports.createCoHost = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth?.uid)
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", err?.message || "Could not create co-host account.");
    }
    const db = admin.firestore();
    ...
    try {
        ...
        throw new https_1.HttpsError("internal", "Account created in Auth but profile save failed. Please retry.");
    }
    return {
        uid: userRecord.uid,
        name,
...
exports.deleteCoHost = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth?.uid)
        throw new https_1.HttpsError("unauthenticated", "Login required");
    await assertFullAdmin(request.auth.uid);
    ...
    if (uid === request.auth.uid) {
        throw new https_1.HttpsError("invalid-argument", "You cannot delete your own account here.");
    }
    const db = admin.firestore();
    const adminSnap = await db.collection("admins").doc(uid).get();
    if (!adminSnap.exists)
        throw new https_1.HttpsError("not-found", "Co-host not found.");
    const data = adminSnap.data();
    if (data.kind !== "cohost") {
        throw new https_1.HttpsError("failed-precondition", "Only co-host accounts can be deleted from this page.");
    }
...
exports.resetCoHostPassword = (0, https_1.onCall)({ cors: true }, async (request) => {
    if (!request.auth?.uid)
        throw new https_1.HttpsError("unauthenticated", "Login required");
    await assertFullAdmin(request.auth.uid);
    const uid = String(request.data?.uid || "").trim();
    ...
    const data = adminSnap.data();
    if (data.kind !== "cohost") {
        throw new https_1.HttpsError("failed-precondition", "Only co-host passwords can be reset here.");
    }
